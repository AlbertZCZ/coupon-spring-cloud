...
import static org.study.coupon.customer.constant.TRAFFIC_VERSION;
...
public class CouponCustomerServiceImpl implements CouponCustomerService {
  ...
  public SimulationResponse simulateOrderPrice(SimulationOrder order) {
    ...
    return webClientBuilder.build().post()
        .uri("http://loadbalancer-coupon-calculation-serv/calculator/simulate")
        .bodyValue(order)
        .retrieve()
  ...
  public List<CouponInfo> findCoupon(SearchCoupon request) {
    ...
    Map<Long, CouponTemplateInfo> templateMap = webClientBuilder.build().get()
        .uri("http://loadbalancer-coupon-template-serv/template/getBatch?ids=" + templateIds)
        .retrieve()
        // 设置返回值类型
        .bodyToMono(new ParameterizedTypeReference<Map<Long, CouponTemplateInfo>>() {})
        .block();
    coupons.forEach(e -> {
      assert templateMap != null;
      e.setTemplateInfo(templateMap.get(e.getTemplateId()));
    });
  ...
  public Coupon requestCoupon(RequestCoupon request) {
    CouponTemplateInfo templateInfo = webClientBuilder.build()
        // 声明了这是一个GET方法
        .get()
        .uri("http://loadbalancer-coupon-template-serv/template/getTemplate?id=" + request.getCouponTemplateId())
  ...
  public ShoppingCart placeOrder(ShoppingCart order) {
    ...
    ShoppingCart checkoutInfo = webClientBuilder.build().post()
        .uri("http://loadbalancer-coupon-calculation-serv/calculator/checkout")
        .bodyValue(order)
        .retrieve()
  ...
  private CouponTemplateInfo loadTemplateInfo(Long templateId) {
    return webClientBuilder.build().get()
        .uri("http://loadbalancer-coupon-template-serv/template/getTemplate?id=" + templateId)
        .retrieve()
        .bodyToMono(CouponTemplateInfo.class)
        .block();
  }
